...
import {
  ...
  Package, Wallet, HelpCircle, LogOut,
} from "lucide-react";
...
export function SupplierSidebar({ userName, userEmail }: {
  userName?: string;
  userEmail?: string;
}) {
    ...
    >
      ...
      >
        ...
        >
          <HelpCircle
            className="w-[17px] h-[17px] flex-shrink-0"
            style={{ color: pathname.startsWith("/supplier/notifications") ? "#4361EE" : "#9CA3AF" }}
          />
          Support
        </Link>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-semibold truncate leading-tight" style={{ color: "#0C1220" }}>
              {userName || "Supplier"}
            </p>
            <p className="text-[11px] truncate leading-tight" style={{ color: "#9CA3AF" }}>
              {userEmail || "Supplier Account"}
            </p>
          </div>
          <button
            onClick={e => {
